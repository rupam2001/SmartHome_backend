...
app.post("/get_devices", async (req, res) => {
  const { accessToken } = req.body;
  console.log(accessToken, "get devices called");
  try {
    const { user_id } = await getAuthDataFromAccessToken(accessToken);
    let devices = await DEVICE.find({ user_id: user_id }).populate("room_id");
    //removing the unneccessary switches
    for (let i = 0; i < devices.length; i++) {
      devices[i].switches = devices[i].switches.slice(
        3,
        devices[i].switches.length
      );
    }
    ...
    for (let i = 0; i < _devices.length; i++) {
      ...
      if (!device_ws_map) {
        ...
        await redisClient.set(
          device_id,
          JSON.stringify({ clients: [wstoken] })
        ); //i think we can elemenate await
      } else {
        device_ws_map = JSON.parse(device_ws_map);
        await redisClient.set(
          device_id,
          JSON.stringify({ clients: [...device_ws_map.clients, wstoken] })
        );
      }
